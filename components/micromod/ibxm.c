...
struct module* module_load( struct data *data, char *message ) {
	...
	if( !memcmp( data_ascii( data, 0, 16, ascii ), "Extended Module:", 16 ) ) {
		...
		/* Impulse Tracker format not supported by IBXM */
		strcpy( message, "IT format not supported" );
		module = NULL;
	} else {
...
/* ESP-IDF extensions for MOD backend */

int replay_get_tempo( struct replay *replay ) {
	return replay->tempo;
}

int replay_get_speed( struct replay *replay ) {
	return replay->speed;
}

struct module* replay_get_module( struct replay *replay ) {
	return replay->module;
}

int pattern_get_note_data( struct pattern *pattern, int row, int channel,
    unsigned char *out_key, unsigned char *out_ins,
    unsigned char *out_vol, unsigned char *out_fxt, unsigned char *out_fxp ) {
	int offset;
	if( pattern == NULL || row < 0 || row >= pattern->num_rows ||
	    channel < 0 || channel >= pattern->num_channels ) {
		return -1;
	}
	offset = ( row * pattern->num_channels + channel ) * 5;
	if( out_key ) *out_key = pattern->data[ offset ];
	if( out_ins ) *out_ins = pattern->data[ offset + 1 ];
	if( out_vol ) *out_vol = pattern->data[ offset + 2 ];
	if( out_fxt ) *out_fxt = pattern->data[ offset + 3 ];
	if( out_fxp ) *out_fxp = pattern->data[ offset + 4 ];
	return 0;
}
