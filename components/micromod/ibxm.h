...
/* Returns the current speed (ticks per row). */
int replay_get_speed( struct replay *replay );
/* Returns the module associated with this replay. */
struct module* replay_get_module( struct replay *replay );
/* Get pattern note data at specified position.
   Returns 0 on success, -1 on error.
   out_key: note (0=none, 1-96=note, 97=noteoff)
   out_ins: instrument (0=none, 1+=instrument)
   out_vol: volume column
   out_fxt: effect type
   out_fxp: effect parameter */
